...
/**
 * Returns a localized error message string for a given [AnalysisError].
 *
 * Maps specific [RepositoryError] types to corresponding string resources.
 *
 * @param error The [AnalysisError] to display a message for.
 * @return The localized error message string.
 */
@Composable
internal fun getRepositoryErrorMessage(error: AnalysisError): String {
    val resourceId =
        when (error) {
